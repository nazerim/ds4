...
3. **Divergence anchors:** implement §7.2; stub test: miss at C with A<C stores a C-anchor; second identical miss loads C; cap enforced. — **DONE**: `ds4_kvstore_set_divergence_target` + per-slot `divergence_target_tokens`; fired by `kv_cache_maybe_store_continued` (server path) once the live session reaches the target; grid-aligned targets skipped (covered by continued store); `--kv-cache-max-divergence-anchors` (default 8, 0=off). New test: `test_kv_cache_divergence_target_logic`.
